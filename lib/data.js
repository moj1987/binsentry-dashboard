...
export async function getInventory() {
  try {
    ...
    // Convert DynamoDB format to plain JSON with correct field names
    return response.Items.map(item => ({
      id: item.id.S,
      feedType: item.feedType?.S || item.name?.S || 'Unknown',
      quantity: parseInt(item.quantity.N),
      binId: item.binId.S,
      entryDate: item.entryDate?.S || item.lastUpdated?.S || new Date().toISOString(),
      notes: item.notes?.S || ''
    }));
  } catch (error) {
